...
class PurgeMessages extends commando.Command{
    ...
    async run(message,args){
        ...
        var intRegex=/^\d+$/
        if (args[0]=="help")
            message.channel.send("Help for `&purge`: You can specify the amount of messages you want the bot to delete.\nExample: `&purge 10`\nExample in DMs (this will only delete the bot's messages, not your own): `@OpenScykohBot purge 10` or simply `purge 10`")
        if(!args&&(message.member.hasPermission('MANAGE_MESSAGES')||message.guild.me.hasPermission('MANAGE_MESSAGES'))){
            message.delete()
            message.channel.send("You need to specify the amount of messages to delete.\nExample: `&purge 10`\nExample in DMs (this will only delete the bot's messages, not your own): `@OpenScykohBot purge 10` or simply `purge 10`").then(message=>message.delete(3000))
            return
        }else if((!intRegex.test(args)||intRegex.test(args)&&args<=0)&&(message.member.hasPermission('MANAGE_MESSAGES')||message.guild.me.hasPermission('MANAGE_MESSAGES'))){
            message.delete()
            ...
            if(args&&(!message.member.hasPermission('MANAGE_MESSAGES')||!message.guild.me.hasPermission('MANAGE_MESSAGES'))){
                message.reply('Either you, the bot, or both are missing the MANAGE_MESSAGES permission.').then(message=>message.delete(3000))
            ...
            if(args>100){
                message.delete()
                message.channel.send("I can only delete up to 100 of my DM messages at a time.\nLower the message amount and try again, as you're probably gonna overload both me and the API.\nNo one wants that, do we?").then(message=>message.delete(3000))
            }else{
                await message.channel.fetchMessages().then(message=>{message.filter(message=>message.author.id===this.client.user.id).first(parseInt(args)).map(message=>message.delete())})
            }
            if(args==1){
