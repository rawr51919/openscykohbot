...
class BCRNG extends commando.Command {
    ...
    async run(message, args) {
        ...
        if ((message.content.match(bot.commandPrefix+/^bcrng$/)) || (message.content.match(message.guild.commandPrefix+/^bcrng$/))){
        ...
        }else if ((message.content.match(bot.commandPrefix+/^bcrng ([0-9]*) ([0-9]*)+\b/) && !number[2]) || (message.content.match(message.guild.commandPrefix+/^bcrng ([0-9]*) ([0-9]*)+\b/) && !number[2])){
            message.reply('your generated number is '+random.integer(number[0],number[1])+'.')
        }else if ((message.content.match(bot.commandPrefix+/^bcrng ([0-9]*) ([0-9]*) ([0-9]*)+\b/)) || (message.content.match(message.guild.commandPrefix+/^bcrng ([0-9]*) ([0-9]*) ([0-9]*)+\b/))){
            var numberarray=[]
            var numbertotal=0
