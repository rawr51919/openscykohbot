...
class NMRNG extends commando.Command {
    ...
    async run(message, args) {
        ...
        if ((message.content.match(this.bot.commandPrefix+/^nmrng$/)) || (message.content.match(message.guild.commandPrefix+/^nmrng$/))){
        ...
        }else if ((message.content.match(this.bot.commandPrefix+/^nmrng ([0-9]*) ([0-9]*)+\b/) && !number[2]) || (message.content.match(message.guild.commandPrefix+/^nmrng ([0-9]*) ([0-9]*)+\b/) && !number[2])){
            message.reply('your generated number is '+random.integer(number[0],number[1])+'.')
        }else if ((message.content.match(this.bot.commandPrefix+/^nmrng ([0-9]*) ([0-9]*) ([0-9]*)+\b/)) || (message.content.match(message.guild.commandPrefix+/^nmrng ([0-9]*) ([0-9]*) ([0-9]*)+\b/))){
            var numberarray=[]
            var numbertotal=0
