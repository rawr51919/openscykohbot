...
const DEFAULT_COOLDOWN = 5000; // default cooldown in milliseconds
...
module.exports = {
  ...
  async execute(message, args, serverSettings = {}) {
    ...
    const cooldownTime = serverSettings.knucclesCooldown ?? DEFAULT_COOLDOWN;
    ...
    if (now - lastUsed < cooldownTime) {
      ...
      return message.channel.send(
        `Please wait ${remaining} more second${remaining === 1 ? "" : "s"} before summoning more knuccles.`
      );
    }
    ...
    const images = [
      ...
      "https://i.imgur.com/ZsctD9L.png\nImage originally requested by: `chloerawr#5011`",
    ...
    const selectedImage = images[random.integer(0, images.length - 1)];
    const selectedQuote = quotes[random.integer(0, quotes.length - 1)];

    const combinedMessage = `${selectedImage}\n\n${selectedQuote}`;
